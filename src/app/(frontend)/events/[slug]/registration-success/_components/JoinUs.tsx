...
export default function JoinUs({instructor,startTime,endTime,date,title}) {
      ...
      <div  className="relative -top-16 flex gap-4 sm:gap-10 p-2 sm:p-6 rounded-2xl bg-secondary-100 m-10 max-lg:flex-col max-sm:m-2">
        <div className='flex flex-col items-center gap-2'>
          <div className="w-32  aspect-square bg-red-500 relative rounded-full">
            <Image src={instructor?.profileImage?.url ?? '/placeHolders/NexusBerry_favi.jpg'} alt="alt" fill className="object-cover rounded-full" sizes="130px" />
          </div>
          <h1 className='font-semibold text-center'>{instructor?.name ?? "NexusBerry"}</h1>
        ...
        <div className="flex flex-col space-y-4 sm:space-y-8 px-2 sm:px-4  py-6 max-lg:items-center">
          ...
          <p className="text-2xl   font-semibold max-lg:text-center">
            If you will passionate about building a career in This can be your chance!
          </p>
          ...
          <div className="flex  items-center gap-2 max-lg:justify-center">
            ...
            <span>{format(new Date(startTime), 'h:mm a')} {endTime ?  `- ${format(new Date(endTime), 'h:mm a')}`:''}</span>
          </div>
          </div>
        </div>
