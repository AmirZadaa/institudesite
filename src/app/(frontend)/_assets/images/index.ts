
import android from './android.png'
import flutter from './flutter.jpg'
import Asp from './Asp.jpg'
import dataAnalytics from './dataAnalytics.png'
import Django from './Django.jpg'
import dl from './dl.jpg'
import Dsml from './Dsml.jpg'
import flutter2 from './flutter2.png'
import graphic from './graphic_design.jpg'
import javascript from './javascript.jpg'
import mean from './mean.jpg'
import Mern from './Mern.jpg'
import nextjs from './nextjs.jpg'
import PhpLaravel from './PhpLaravel.jpg'
import python from './python.jpg'
import react from './Reactjpg.jpg'
import reactNative from './reactNative.jpg'
import seo from './seo.jpg'
import social_media from './social_media.jpg'
import social from './social.jpg'
import unity_game from './unity_game.jpg'
import webdesigning from './webdesigning.jpg'
import wordpress from './wordpress.jpg'
import power_bi from './Power_bi.jpg'
import RanaAjmal from './Rana-Ajmal.png'
import thambnail from './thambnail.jpg'
import HeroBgTmg from './herobackImg.webp'
import beginer from './beginer.svg'
import heroCourse2 from './heroCourse2.webp'
import bgImg from './bg-img.webp'

import eventsBanner from './eventBanner.svg'
import data from './data.webp'
import master3 from './master3.webp'
import masterNew from './masterNew.webp'
import masterclass from './masterclass.webp'
import whtp1 from './whatsapp-bg-1.svg'
import whtp2 from './whatsapp-bg-2.svg'
import whtp3 from './whatsapp-bg-3.svg'
import whtp4 from './whatsapp-bg-4.svg'
import scan from './masterclass-whatsapp-channel.svg'
import learning from './learning.webp'
import Certificate from './certificate.svg'
import mentor from './mentor2.webp'
import mentor2 from './mentor2.webp'
import ourMentor from './our-mentor.svg'
import dataVisual from './dataVisualisation.webp'
import whtsppIcon from './whatsapp-icon.svg'
import Majid from './Majid.jpg'
import Map from './map.jpg'

import MearnHomeImg from './MearnHomeImg.jpg'
import MLHomeImg from './MLHomeImg.jpg'
import FlutterHomeImg from './FlutterHomeImg.jpg'

import adobeLogo from './adobeLogo.jpg'
import DjangoLogo from './DjangoLogo.jpg'
import FlutterLogo from './FlutterLogo.jpg'
import JsLogo from './JsLogo.jpg'
import NextLogo from './NextLogo.jpg'
import NodejsLogo from './NodejsLogo.jpg'
import powerbiLogo from './powerbiLogo.jpg'
import pythonLogo from './pythonLogo.jpg'
import ReactLogo from './ReactLogo.jpg'
import tensorFlowLogo from './tensorFlowLogo.jpg'

import twitter from './twitter.svg'
import youtube from './youtub.svg'
import facebook from './facebook.svg'
import instagram from './insta.svg'
import modulebg from './learning-bg.svg'
import module1 from './module1.svg'
import module2 from './module2.svg'
import module3 from './module3.svg'
import module4 from './module4.svg'
import module5 from './module5.svg'
import module6 from './module6.svg'
import placeholderImg from './placeholderImg.jpg'
import login_hero from './login-hero.svg'
import certification from './certificate-icon.svg'

import interactiveLearning from './1.png'
import extraLearn from './2.png'
import Quz from './3.png'
import certificate from './4.png'
import RegisterForFreeImg from './RegisterForFreeImg.jpg'
import register from './register.png'


export {
  android, flutter, Asp, dataAnalytics, Django, dl, Dsml,
  flutter2, graphic, javascript, mean, Mern,
  nextjs, PhpLaravel, python, react, reactNative, seo,
  social_media, social, unity_game, webdesigning, wordpress, power_bi,
  RanaAjmal, thambnail, HeroBgTmg, beginer,
  eventsBanner, data, master3, masterNew, masterclass,
  whtp1, whtp2, whtp3, whtp4, scan, learning, Certificate, mentor, mentor2,
  ourMentor, dataVisual, whtsppIcon,register,
  MearnHomeImg, MLHomeImg, FlutterHomeImg, Map, twitter, facebook, instagram, youtube,
  adobeLogo, DjangoLogo, FlutterLogo, JsLogo, NextLogo, NodejsLogo, powerbiLogo, pythonLogo, ReactLogo, tensorFlowLogo,
  heroCourse2, bgImg, Majid,certification,interactiveLearning,extraLearn,Quz,certificate,
  module1, module2, module3, module4, module5, module6, modulebg,placeholderImg,login_hero,RegisterForFreeImg
}




