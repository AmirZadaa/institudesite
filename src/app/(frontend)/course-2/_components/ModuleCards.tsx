...
import { Django } from '@/app/(frontend)/_assets/images'
    ...
    return (
        <div className='bg-background my-10 max-lg:my-4'>
            <div className='container max-md:px-4 mx-auto text-center lg:space-y-3'>
                ...
                <h2 className='text-5xl max-md:text-3xl max-xs:text-xl max-sm:leading-5 font-bold'>Here&apos;s what you will learn in this Entire Course of 8 to 10 Months</h2>
                <div className='grid grid-cols-4 max-lg:grid-cols-3 max-md:grid-cols-2 max-xs:grid-cols-1 max-md:py-6 gap-8'>
                    {cardData.map((data, i) => (
                            <Card key={data.id} data={data} index={i} />
                    ))}
                </div>
            </div>
        </div>
